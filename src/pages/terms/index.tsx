...
export function Terms() {
  ...
  useEffect(() => {
    const fetchCms = async () => {
      try {
        const response = await api.get<{ title: string; content: string; isActive: boolean }>('/cms/slug/terms');
  ...
  return (
    <LegalLayout
      ...
      subtitle={cmsContent ? "Dynamic platform terms and conditions." : "Last updated: October 24, 2024. Please read these terms carefully before booking."}
    >
      {cmsContent ? (
        <div className="whitespace-pre-line text-sm leading-relaxed text-on-surface-variant font-body">
          {cmsContent.content}
        </div>
      ) : (
        <>
