...
export function AdminDashboard() {
  ...
  const fetchDashboardData = async (showLoader = true) => {
    ...
    try {
      // Fire all 4 requests in parallel — KPI endpoint is cached, so it responds in <50ms on revisit
      ...
      const isCyber = (action: string) =>
        action.includes('FAILED') || action.startsWith('AUTH_') || action === 'PASSWORD_CHANGE';

      const mappedActivities = activityRes.data.map((log: any) => ({
        id: log.id,
        action: log.action,
        type: isCyber(log.action)
          ? 'security'
          : log.action.toLowerCase().includes('login') ? 'user'
          : log.action.toLowerCase().includes('booking') ? 'booking'
          : 'system',
        description: log.details
          ? log.details
          : `${log.user ? `${log.user.firstName} ${log.user.lastName}` : 'Anonymous'}: ${log.action.replace(/_/g, ' ')}`,
        createdAt: new Date(log.createdAt).toLocaleString([], { dateStyle: 'short', timeStyle: 'short' }),
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col">
                  <div className="flex justify-between items-center mb-5">
                    ...
                    <Link to="/admin/activities" className="text-primary text-xs font-bold hover:underline print:hidden flex items-center gap-1">
                  ...
                  <div className="space-y-3 overflow-y-auto max-h-[360px] pr-1">
                    {filteredActivities.length === 0 ? (
                      ...
                      filteredActivities.map((act) => {
                        const isCyberAct = act.type === 'security';
                        return (
                          <div key={act.id} className={`flex gap-3 p-3 rounded-xl border transition-all ${
                            isCyberAct
                              ? 'bg-red-50 border-red-200'
                              : act.type === 'booking' ? 'bg-indigo-50/40 border-indigo-100'
                              : act.type === 'user' ? 'bg-emerald-50/40 border-emerald-100'
                              : 'bg-slate-50 border-slate-100'
                          }`}>
                            <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 ${
                              isCyberAct ? 'bg-red-100 text-red-600'
                              : act.type === 'booking' ? 'bg-indigo-100 text-indigo-600'
                              : act.type === 'user' ? 'bg-emerald-100 text-emerald-600'
                              : 'bg-slate-100 text-slate-500'
                            }`}>
                              {isCyberAct ? <ShieldAlert className="w-3.5 h-3.5" />
                              : act.type === 'booking' ? <CheckSquare className="w-3.5 h-3.5" />
                              : act.type === 'user' ? <LogIn className="w-3.5 h-3.5" />
                              : <ActivityIcon className="w-3.5 h-3.5" />}
                            </div>
                            <div className="flex-1 min-w-0">
                              <p className="text-xs font-semibold text-on-surface leading-tight truncate">{act.description}</p>
                              <div className="flex items-center gap-2 mt-0.5">
                                <span className="text-[10px] text-slate-400 font-medium">{act.createdAt}</span>
                                {act.ipAddress && (
                                  <span className="text-[10px] text-slate-400 flex items-center gap-0.5">
                                    <Globe className="w-2.5 h-2.5" />{act.ipAddress}
                                  </span>
                                )}
                                {isCyberAct && (
                                  <span className="text-[9px] bg-red-100 text-red-600 px-1.5 py-0.5 rounded font-bold uppercase">⚠ Security</span>
                                )}
                              </div>
                            </div>
                          </div>
